...
mpc_dfs = {}
for event in events:
    filename = f"results/storage_estimates_2006/res_df_mpc_{event}_weight{weight}.csv"
    mpc_dfs[event] = pd.read_csv(filename, index_col=0)

# hefs trained, fully and held out
hefs_dfs = {}
for event in events:
    filename = f"results/storage_estimates_2006/res_df_hefs_{event}_weight{weight}.csv"
    hefs_dfs[event] = pd.read_csv(filename, index_col=0)

# syn fully trained
syn_hefs_dfs = {}
for event in events:
    filename = f"results/storage_estimates_2006/res_df_syn_train_{event}_weight{weight}.csv"
    syn_hefs_dfs[event] = pd.read_csv(filename, index_col=0)

# syn trained held out
syn_held_dfs = {}
for event in events:
    filename = f"results/storage_estimates_2006/res_df_syn_train_no1997_{event}_weight{weight}.csv"
...
cumulative_dfs = {}
for event in events:
    filename = f"results/storage_estimates_2006/res_df_cumulative_{event}.csv"
...
ax.set_xticklabels(['2006 Flood','100 Year Scaled','200 Year Scaled','300 Year Scaled'])
ax.set_ylim(-5,1700)
ax.set_xlim(-0.5,3.5)
ax.annotate(text=('Scaled Events'), xy=(2,1600), ha='center', fontweight='bold', fontsize=12)
ax.annotate(text=('Observed Event'), xy=(0,1600), ha='center', fontweight='bold', fontsize=12)
...
fig.suptitle(f'Required Flood Pool by Policy - 2006 Flood, Weight: {weight}', fontweight='bold', fontsize=18)
