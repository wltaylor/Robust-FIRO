...
mpc_dfs = {}
for event in events:
    filename = f"results/storage_estimates_1997/res_df_mpc_{event}_weight{weight}.csv"
    mpc_dfs[event] = pd.read_csv(filename, index_col=0)

# hefs trained, fully and held out
hefs_dfs = {}
for event in events:
    filename = f"results/storage_estimates_1997/res_df_hefs_{event}_weight{weight}.csv"
    hefs_dfs[event] = pd.read_csv(filename, index_col=0)

# syn fully trained
syn_hefs_dfs = {}
for event in events:
    filename = f"results/storage_estimates_1997/res_df_syn_train_{event}_weight{weight}.csv"
    syn_hefs_dfs[event] = pd.read_csv(filename, index_col=0)

# syn trained held out
syn_held_dfs = {}
for event in events:
    filename = f"results/storage_estimates_1997/res_df_syn_train_no1997_{event}_weight{weight}.csv"
...
cumulative_dfs = {}
for event in events:
    filename = f"results/storage_estimates_1997/res_df_cumulative_{event}.csv"
...
boxplot_handles = [Line2D([0], [0], color=color, lw=4) for color in colors]

# Combine scatter and boxplot handles
#scatter_handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color, markersize=8) for color in colors[:5]]
#legend_handles = scatter_handles + boxplot_handles[5:]  # Adjust this to match your scatter and boxplot combinations
...
ax.set_xticklabels(['1997 Flood','100 Year Scaled','200 Year Scaled','300 Year Scaled'])
ax.set_ylim(-5,1500)
ax.set_xlim(-0.5,3.5)
ax.annotate(text=('Scaled Events'), xy=(2,1400), ha='center', fontweight='bold', fontsize=12)
ax.annotate(text=('Observed Event'), xy=(0,1400), ha='center', fontweight='bold', fontsize=12)
ax.axvline(x=0.5, color='black', linestyle='--', linewidth=1)
#ax.axvline(x=1.5, color='gray', linestyle='dashdot', linewidth=1)
#ax.axvline(x=2.5, color='gray', linestyle='dotted', linewidth=1)

for spine in ax.spines.values():
    spine.set_edgecolor('black')
    spine.set_linewidth(1)

#fig.suptitle(f'Required Flood Pool by Policy - 1997 Flood, Weight: {weight}', fontweight='bold', fontsize=18)
#plt.title(f'Required Flood Pool by Policy', fontweight='bold', fontsize=18)
...
fig.legend(handles=[legend_baseline, legend_perf, legend_MPC, legend_EFO, legend_EFO_held, legend_syn, legend_syn_held, legend_cumulative, legend_scalar, legend_boxplot], loc='upper center', bbox_to_anchor=(0.52, -0.01), ncol=4, edgecolor='black')


#legend = ax.legend(handles=legend_handles, labels=handles, bbox_to_anchor=(1.01, 0.95), title='Policies', title_fontproperties={'weight': 'bold'})
#legend.get_frame().set_edgecolor('black')  # Optional: add a border to the legend
plt.tight_layout()
plt.savefig('/Users/williamtaylor/Documents/Github/Robust-FIRO/figures/figure_4.pdf', format='pdf', bbox_inches='tight', transparent=False)
plt.show()
